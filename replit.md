...
- June 17, 2025. FASE 1 CORE SOLID INIZIATA: Implementato Auth Manager enterprise-grade con JWT refresh tokens, token families per sicurezza avanzata, token theft detection automatico, storage Redis-like in-memory con cleanup. Transaction Manager per operazioni atomiche multi-tabella (registrazioni, verifiche documenti, pagamenti). Avvio implementazione Error Boundaries e File Upload Cloud per completare i 4 pilastri della base solida
