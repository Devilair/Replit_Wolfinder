...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Update user profile
  app.patch("/api/users/profile", authService.authenticateToken, async (req: any, res) => {
    try {
      const user = req.user;
  ...
  // Submit a review (authenticated or anonymous)
  app.post("/api/reviews", async (req, res) => {
    try {
      const { professionalId, rating, comment, serviceUsed, reviewerName, reviewerEmail } = req.body;
      
      // Basic validation
      if (!professionalId || !rating || !comment || !serviceUsed) {
        return res.status(400).json({ message: "Missing required fields" });
      }
      ...
      // For anonymous reviews, require name and email
      if (!userId && (!reviewerName || !reviewerEmail)) {
        return res.status(400).json({ message: "Name and email required for anonymous reviews" });
      }
      ...
      const reviewData: any = {
        ...
        text: comment,
        serviceUsed,
        status: userId ? 'pending' : 'pending_verification', // Authenticated reviews go to moderation, anonymous need email verification
        userId: userId || null,
        reviewerName: userId ? null : reviewerName,
        reviewerEmail: userId ? null : reviewerEmail,
        createdAt: new Date(),
        updatedAt: new Date()
      };
      ...
      // For anonymous reviews, send verification email
      if (!userId && reviewerEmail) {
        // TODO: Implement email verification for anonymous reviews
        // For now, we'll mark them as pending verification
      }
  ...
  // Delete/anonymize user review
  app.delete("/api/reviews/:reviewId", authService.authenticateToken, async (req: any, res) => {
    try {
      const user = req.user;
      ...
      await storage.updateReview(reviewId, {
        text: "[Recensione rimossa dall'utente]",
        status: 'deleted'
      });

      res.json({ message: "Review anonymized successfully" });
    } catch (error) {
      console.error("Error deleting review:", error);
      res.status(500).json({ message: "Failed to delete review" });
    }
